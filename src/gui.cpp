...
// callbacks
    ...
    static std::map<std::string, std::pair<void (*)(void *), void *>> gui_callbacks_arg;

    void gui_add_callback(const std::string &name, void (*fn)(void)) {
        gui_callbacks[name] = fn;
    }

    void gui_add_callback(const std::string &name, void (*fn)(void *), void *arg) {
        std::pair<void (*)(void *), void *> p{fn, arg};
        gui_callbacks_arg[name] = p;
    }

    void gui_remove_callback(const std::string &name) {
        if (gui_callbacks.count(name) > 0)
            gui_callbacks.erase(name);
        if (gui_callbacks_arg.count(name) > 0)
            gui_callbacks_arg.erase(name);
    }
...
// main setup, draw and shutdown routines
    ...
    void gui_draw() {
        ...
        static bool gui_show_timers = true;
        static bool gui_show_debug = false;

        if (ImGui::BeginMainMenuBar()) {
            ImGui::Checkbox("debug", &gui_show_debug);
            ImGui::Separator();
        ...
        if (gui_show_debug) {
            // camera menu
            if (ImGui::Begin("Debug info", &gui_show_debug))
            ...
            ImGui::Separator();
            ImGui::Checkbox("timers", &gui_show_timers);
            ImGui::Separator();

            ImGui::End();
        }
        if (gui_show_cameras) {
            if (ImGui::Begin(std::string("Cameras (" + std::to_string(Camera::map.size()) + ")").c_str(),
                             &gui_show_cameras)) {
        ...
        if (gui_show_textures) {
            if (ImGui::Begin(std::string("Textures (" + std::to_string(Texture2D::map.size()) + ")").c_str(),
                             &gui_show_textures)) {
                for (const auto &[name, tex]: Texture2D::map) {
                    if (ImGui::CollapsingHeader(name.c_str()))
                        gui_display_texture(tex, ivec2(300, 300));
                }
            }
            ImGui::End();
        }
        ...
        if (gui_show_shaders) {
            if (ImGui::Begin(std::string("Shaders (" + std::to_string(Shader::map.size()) + ")").c_str(),
                             &gui_show_shaders)) {
                for (auto &[name, shader]: Shader::map)
                    if (ImGui::CollapsingHeader(name.c_str()))
                        gui_display_shader(shader);
                if (ImGui::Button("Reload modified"))
        ...
        if (gui_show_fbos) {
            if (ImGui::Begin(std::string("Framebuffers (" + std::to_string(Framebuffer::map.size()) + ")").c_str(),
                             &gui_show_fbos)) {
        ...
        if (gui_show_materials) {
            if (ImGui::Begin(std::string("Materials (" + std::to_string(Material::map.size()) + ")").c_str(),
                             &gui_show_materials)) {
        ...
        if (gui_show_geometries) {
            if (ImGui::Begin(std::string("Geometries (" + std::to_string(Geometry::map.size()) + ")").c_str(),
                             &gui_show_geometries)) {
        ...
        if (gui_show_drawelements) {
            if (ImGui::Begin(std::string("Drawelements (" + std::to_string(Drawelement::map.size()) + ")").c_str(),
                             &gui_show_drawelements)) {
        ...
        if (gui_show_animations) {
            if (ImGui::Begin(std::string("Animation (" + std::to_string(Animation::map.size()) + ")").c_str(),
                             &gui_show_animations)) {
    ...
    void gui_draw_callbacks() {
        ...
        for (const auto &[name, fn]: gui_callbacks)
            fn();
        for (const auto &[name, fn]: gui_callbacks_arg)
            fn.first(fn.second);
    }
...
// helpers
    ...
    void gui_display_texture(const Texture2D &tex, const ivec2 &size) {
        ImGui::Indent();
        ...
        ImGui::Image((ImTextureID) size_t(tex->id), ImVec2(size.x(), size.y()), ImVec2(0, 1), ImVec2(1, 0),
                     ImVec4(1, 1, 1, 1), ImVec4(1, 1, 1, 0.5));
        if (ImGui::Button(("Save PNG##" + tex->name).c_str()))
            tex->save_ldr(fs::path(tex->name).filename().replace_extension(".png"));
        ImGui::SameLine();
        if (ImGui::Button(("Save JPEG##" + tex->name).c_str()))
            tex->save_ldr(fs::path(tex->name).filename().replace_extension(".jpg"));
        ImGui::Unindent();
    }
    ...
    void gui_display_framebuffer(const Framebuffer &fbo) {
        ...
        for (uint32_t i = 0; i < fbo->color_textures.size(); ++i)
            if (ImGui::CollapsingHeader(
                    std::string("color attachment " + std::to_string(i) + "##" + fbo->name).c_str()))
    ...
    void gui_display_material(const Material &mat) {
        ...
        for (const auto &entry: mat->vec4_map)
            ImGui::Text("%s: (%f, %f, %f, %.f)", entry.first.c_str(), entry.second.x(), entry.second.y(),
                        entry.second.z(), entry.second.w());
        ImGui::Unindent();
    ...
    void gui_display_mat4(mat4 &mat) {
        ...
        bool modified = false;
        if (ImGui::DragFloat4("row0", &row_maj(0, 0), .01f))
            modified = true;
        if (ImGui::DragFloat4("row1", &row_maj(0, 1), .01f))
            modified = true;
        if (ImGui::DragFloat4("row2", &row_maj(0, 2), .01f))
            modified = true;
        if (ImGui::DragFloat4("row3", &row_maj(0, 3), .01f))
            modified = true;
        if (modified)
            mat = transpose(row_maj);
        ImGui::Unindent();
    }
    ...
    void gui_display_animation(const Animation &anim) {
        ...
        ImGui::Text("curr time: %.3f / %lu, ms_per_node: %.3f", anim->time, anim->camera_path.size(),
                    anim->ms_between_nodes);
        ImGui::Text("camera path:");
        ImGui::Indent();
        for (const auto &[pos, lookat]: anim->camera_path)
            ImGui::Text("pos: (%.3f, %.3f, %.3f), lookat: (%.3f, %.3f, %.3f)", pos.x(), pos.y(), pos.z(), lookat.x(),
                        lookat.y(), lookat.z());
        ImGui::Unindent();
    ...
    void gui_display_query_timer(const Query &query, const char *label) {
        ...
        ImGui::PlotHistogram(label, query.data.data(), query.data.size(), query.curr, 0, 0.f, std::max(upper, 17.f),
                             ImVec2(0, 30));
        ImGui::PopStyleColor();
    }
    ...
    void gui_display_query_counter(const Query &query, const char *label) {
        ...
        const float lower = query.min();
        const float upper = query.max();
        ImGui::Text("avg: %uK, min: %uK, max: %uK", uint32_t(avg / 1000), uint32_t(lower / 1000),
                    uint32_t(upper / 1000));
    ...
    void gui_display_timers() {
        ...
        ImGui::SetNextWindowPos(
                ImVec2(Context::resolution().x() - (350 + 200 * (ImGui::GetIO().FontGlobalScale - 1)), 20));
