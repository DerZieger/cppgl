...
// LineImpl
    ...
    Geometry LineImpl::generate_geometry(
    ...
    LineImpl::LineImpl(const std::string &name, const vec3 &from,
                       const vec3 &to, const Material &material, GLenum hint)
            : MeshImpl(name,
                       LineImpl::generate_geometry("geometry_" + name,
                                                   from, to),
                       material, hint),
              from(from),
              to(to) {
...
// PointCloudImpl
    ...
    Geometry PointCloudImpl::generate_geometry(
            const std::string &name, const std::vector<vec3> &points, const int size) {
        std::vector<vec3> positions;
        std::vector<uint32_t> indices;
        int c = 0;
    ...
    PointCloudImpl::PointCloudImpl(const std::string &name, const std::vector<vec3> &points, const int size,
                                   const Material &material, GLenum hint)
            : MeshImpl(name, PointCloudImpl::generate_geometry("geometry_" + name, points, size), material, hint) {
...
// CuboidImpl
    Geometry CuboidImpl::generate_geometry(
    ...
    CuboidImpl::CuboidImpl(const std::string &name, float width,
                           float height, float depth,
                           const vec3 &center,
                           const Material &material)
            : MeshImpl(name,
                       CuboidImpl::generate_geometry(
    ...
    CuboidImpl::~CuboidImpl() {}

// ------------------------------------------
// SphereImpl

    Geometry SphereImpl::generate_geometry(
        ...
        Geometry geo(name, positions, indices, normals, texcoords);
        return geo;
    }

    SphereImpl::SphereImpl(const std::string &name, float radius,
                           const vec3 &center,
                           const Material &material,
                           float sector_count, float stack_count)
            : MeshImpl(
            name,
            SphereImpl::generate_geometry(
...
// EllipsoidImpl
    ...
    Geometry EllipsoidImpl::generate_geometry(
        ...
        Geometry geo(name, positions, indices, normals);
        return geo;
    }

    EllipsoidImpl::EllipsoidImpl(
            const std::string &name, const vec3 &pc1, const vec3 &pc2, const vec3 &pc3,
            const vec3 &center, const Material &material,
            float sector_count, float stack_count)
            : MeshImpl(name,
                       EllipsoidImpl::generate_geometry(
...
///////////////////////////////////////////////////////////////////////////////
    ...
    Geometry CylinderImpl::generate_geometry(
    ...
    CylinderImpl::CylinderImpl(
            ...
            const Material &material, float sector_count,
            float stack_count)
            : MeshImpl(
            name,
            CylinderImpl::generate_geometry(
...
// FrustumImpl
    ...
    Geometry FrustumImpl::generate_geometry(
    ...
    FrustumImpl::FrustumImpl(
            const std::string &name, const vec3 &pos, const mat4 &view,
            const mat4 &proj, const Material &material)
            : MeshImpl(name,
                       FrustumImpl::generate_geometry(
...
// VoxelGridImpl
    ...
    Geometry VoxelGridImpl::generate_geometry(
    ...
    VoxelGridImpl::VoxelGridImpl(
            const std::string &name, unsigned int size, float scale,
            const Material &material)
            : MeshImpl(name,
                       VoxelGridImpl::generate_geometry(
...
// VoxelGridXYZImpl
    ...
    Geometry VoxelGridXYZImpl::generate_geometry(
    ...
    VoxelGridXYZImpl::VoxelGridXYZImpl(
            const std::string &name, const vec3 &size, float scale,
            const Material &material)
            : MeshImpl(name,
                       VoxelGridXYZImpl::generate_geometry(
                               "geometry_" + name, size, scale),
                       material) {
        primitive_type = GL_LINES;
    }
...
// BoundingBoxImpl
    ...
    Geometry BoundingBoxImpl::generate_geometry(
    ...
    BoundingBoxImpl::BoundingBoxImpl(const std::string &name,
                                     const vec3 &bb_min,
                                     const vec3 &bb_max,
                                     const Material &material)
            : MeshImpl(name,
                       BoundingBoxImpl::generate_geometry(
