...
CPPGL_NAMESPACE_BEGIN
    ...
    void FramebufferImpl::attach_depthbuffer(Texture2D tex, bool with_stencil) {
        ...
        if (!tex)
            tex = Texture2D(name + "_default_depth_" + (with_stencil ? "and_stencil_" : "") + "buffer", w, h,
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, id);
        depth_texture = tex;
        glFramebufferTexture2D(GL_FRAMEBUFFER, with_stencil ? GL_DEPTH_STENCIL_ATTACHMENT : GL_DEPTH_ATTACHMENT,
