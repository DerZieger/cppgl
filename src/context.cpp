...
// helper funcs
    ...
    bool Context::show_gui = false;

    static std::pair<void (*)(int key, int scancode, int action, int mods,
                              void *ptr), void *> user_keyboard_callback = {0, nullptr};
    //static void (*user_keyboard_callback)(int key, int scancode, int action, int mods, void *ptr) = 0;

    static std::pair<void (*)(double xpos, double ypos, void *ptr), void *> user_mouse_callback = {0, nullptr};
    //static void (*user_mouse_callback)(double xpos, double ypos, void *ptr) = 0;

    static std::pair<void (*)(int button, int action, int mods, void *ptr), void *> user_mouse_button_callback = {0,
                                                                                                                  nullptr};
    //static void (*user_mouse_button_callback)(int button, int action, int mods, void *ptr) = 0;

    static std::pair<void (*)(double xoffset, double yoffset, void *ptr), void *> user_mouse_scroll_callback = {0,
                                                                                                                nullptr};
    //static void (*user_mouse_scroll_callback)(double xoffset, double yoffset, void *ptr) = 0;

    static std::pair<void (*)(int w, int h, void *ptr), void *> user_resize_callback = {0, nullptr};
    //static void (*user_resize_callback)(int w, int h, void *ptr) = 0;

    static void glfw_key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
        if (key == GLFW_KEY_F1 && action == GLFW_PRESS)
            Context::show_gui = !Context::show_gui;
        ImGui_ImplGlfw_KeyCallback(window, key, scancode, action, mods);
        if (ImGui::GetIO().WantCaptureKeyboard)
            return;
        if (key == GLFW_KEY_ESCAPE)
            glfwSetWindowShouldClose(window, 1);
        if (user_keyboard_callback.first)
            user_keyboard_callback.first(key, scancode, action, mods, user_keyboard_callback.second);
    }

    static void glfw_mouse_callback(GLFWwindow *window, double xpos, double ypos) {
        if (ImGui::GetIO().WantCaptureMouse)
            return;
        if (user_mouse_callback.first)
            user_mouse_callback.first(xpos, ypos, user_mouse_callback.second);
    }
    ...
    static void glfw_mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
        ...
        if (ImGui::GetIO().WantCaptureMouse)
            return;
        if (user_mouse_button_callback.first)
            user_mouse_button_callback.first(button, action, mods, user_mouse_button_callback.second);
    }
    ...
    static void glfw_mouse_scroll_callback(GLFWwindow *window, double xoffset, double yoffset) {
        ...
        if (user_mouse_scroll_callback.first)
            user_mouse_scroll_callback.first(xoffset, yoffset, user_mouse_scroll_callback.second);
    }

    static void glfw_resize_callback(GLFWwindow *window, int w, int h) {
        glViewport(0, 0, w, h);
        if (user_resize_callback.first)
            user_resize_callback.first(w, h, user_resize_callback.second);
    }
...
// Context
    ...
    void Context::capture_mouse(bool on) {
        glfwSetInputMode(instance().glfw_window, GLFW_CURSOR, on ? GLFW_CURSOR_DISABLED : GLFW_CURSOR_NORMAL);
    }

    void Context::set_attribute(int attribute, bool value) {
        glfwSetWindowAttrib(instance().glfw_window, attribute, value ? GLFW_TRUE : GLFW_FALSE);
    }

    vec2 Context::mouse_pos() {
        double xpos, ypos;
        glfwGetCursorPos(instance().glfw_window, &xpos, &ypos);
        return vec2(xpos, ypos);
    }

    bool Context::mouse_button_pressed(int button) {
    ...
    void Context::set_keyboard_callback(
            void (*fn)(int key, int scancode, int action, int mods, void *ptr), void *ptr) {
        user_keyboard_callback = std::pair<void (*)(int key, int scancode, int action, int mods, void *ptr), void *>(fn,
                                                                                                                     ptr);
    }

    void Context::set_mouse_callback(void (*fn)(double xpos, double ypos, void *ptr), void *ptr) {
        user_mouse_callback = std::pair<void (*)(double xpos, double ypos, void *ptr), void *>(fn, ptr);
    }

    void Context::set_mouse_button_callback(
            void (*fn)(int button, int action, int mods, void *ptr), void *ptr) {
        user_mouse_button_callback = std::pair<void (*)(int button, int action, int mods, void *ptr), void *>(fn, ptr);
    }

    void
    Context::set_mouse_scroll_callback(
            void (*fn)(double xoffset, double yoffset, void *ptr), void *ptr) {
        user_mouse_scroll_callback = std::pair<void (*)(double xoffset, double yoffset, void *ptr), void *>(fn, ptr);
    }

    void Context::set_resize_callback(void (*fn)(int w, int h, void *ptr), void *ptr) {
        user_resize_callback = std::pair<void (*)(int w, int h, void *ptr), void *>(fn, ptr);
    }
