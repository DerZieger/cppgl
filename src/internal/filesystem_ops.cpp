#include <internal/filesystem_ops.h>

#include <iostream>

void cppgl::utils::CreateDirectories(const fs::path &path) {
...
void cppgl::utils::CreateDirectories(const std::vector<fs::path> &paths) {
    for (unsigned int i = 0; i < paths.size(); i++) {
        CreateDirectories(paths[i]);
    }
}

fs::path cppgl::utils::SubtractPaths(const fs::path &upper,
                                     const fs::path &lower) {
    fs::path diff_path;
    fs::path base = lower;
