...
// Texture2D
    ...
    public:
        ...
        Texture2DImpl(const std::string &name, uint32_t w, uint32_t h, GLint internal_format, GLenum format,
                      GLenum type,
    ...
    template
    class _API NamedHandle<Texture2DImpl>; // needed for Windows DLL export
...
// Texture3D
    ...
    public:
        // construct empty texture or from raw data
        Texture3DImpl(const std::string &name, uint32_t w, uint32_t h, uint32_t d, GLint internal_format, GLenum format,
                      GLenum type,
                      const void *data = 0, bool mipmap = false);
    ...
    template
    class _API NamedHandle<Texture3DImpl>; // needed for Windows DLL export
