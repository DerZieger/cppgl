...
// Drawelement (object instance for rendering)
    ...
    public:
        ...
        void set_model_transform(const mat4 &trans);
        ...
        void add_pre_draw_func(const std::string &key, std::function<void()> func,
            ...
            if (!overwrite)
                _CPPGL_ASSERT_EQ(pre_draw_funcs.count(key), 0);
        ...
        void add_post_draw_func(const std::string &key, std::function<void()> func,
                                bool overwrite = false) {
            if (!overwrite)
                _CPPGL_ASSERT_EQ(post_draw_funcs.count(key), 0);
    ...
    public:
        ...
        void add_drawelement(Drawelement &d);
