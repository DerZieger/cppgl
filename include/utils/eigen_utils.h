#ifndef CPPGL_UTILS_EIGEN_UTILS_H
#define CPPGL_UTILS_EIGEN_UTILS_H
...
template<typename T>
inline CPPGL_HD Eigen::Matrix<T, 4, 4> MatrixFromEulerTrans(T alpha, T beta,
                                                            T gamma, T tx,
                                                            T ty, T tz) {
...
CPPGL_NAMESPACE_BEGIN
    ...
    inline void printBlocksAboveThresh(const Eigen::MatrixBase<Derived> &mat,
                                       float thresh) {
#if CPPGL_DEBUG_MODE
        _CPPGL_ASSERT_EQ(mat.rows() % BLOCK_ROWS, 0);
        _CPPGL_ASSERT_EQ(mat.cols() % BLOCK_COLS, 0);
#endif
