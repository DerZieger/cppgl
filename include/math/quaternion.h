...
CPPGL_NAMESPACE_BEGIN
    ...
    public:
        CPPGL_HD Quaternion() : coeff(vec4(0.f, 0.f, 0.f, 0.f)) {};
        CPPGL_HD Quaternion(const vec4 &coeff) : coeff(coeff) {};
        CPPGL_HD Quaternion(const vec3 &coeff)
                : coeff(coeff.x(), coeff.y(), coeff.z(), 0) {};
        CPPGL_HD Quaternion(float i, float j, float k, float w)
                : coeff(vec4(i, j, k, w)) {};

        // Generate a quaternion from rotation of a Rt Matrix.
        CPPGL_HOST Quaternion(const mat4 &r) {
        ...
        CPPGL_HD float norm() { return (float) coeff.norm(); }

        CPPGL_HD mat4 toMatrix() const {
        ...
        template<typename T>
        static CPPGL_HOST Eigen::Matrix<T, 4, 4> toMatrix(T const *data) {
        ...
        CPPGL_HD float w() const { return coeff[3]; }

        CPPGL_HD float i() const { return coeff[0]; }

        CPPGL_HD float j() const { return coeff[1]; }

        CPPGL_HD float k() const { return coeff[2]; }

        CPPGL_HD float x() const { return coeff[0]; }

        CPPGL_HD float y() const { return coeff[1]; }

        CPPGL_HD float z() const { return coeff[2]; }

        CPPGL_HD float norm() const { return (float) coeff.norm(); }

        CPPGL_HD vec3 imag() const { return coeff.block<3, 1>(0, 0); }

        CPPGL_HD float real() const { return coeff[3]; }

        CPPGL_HD vec4 data() const { return coeff; }

        CPPGL_HD friend Quaternion operator*(float a,
                                             const Quaternion &q) {
            vec4 newQ = a * q.coeff;
            return Quaternion(newQ[0], newQ[1], newQ[2], newQ[3]);
        }

        CPPGL_HD friend Quaternion operator*(const Quaternion &q,
                                             float a) {
            return a * q;
        }

        CPPGL_HD friend Quaternion operator*(const Quaternion &p,
                                             const Quaternion &q) {
            vec4 r;
        ...
        CPPGL_HD friend vec3 operator*(const Quaternion &p,
                                       const vec3 &q) {
        ...
        CPPGL_HD friend Quaternion operator+(const Quaternion &q1,
                                             const Quaternion &q2) {
        ...
        CPPGL_HD friend Quaternion &operator+=(Quaternion &q1,
                                               const Quaternion &q2) {
            q1.coeff += q2.coeff;
            return q1;
        }
        ...
        CPPGL_HD friend Quaternion &operator/=(Quaternion &q, float a) {
            q.coeff /= a;
            return q;
        }

        CPPGL_HD float operator[](const int i) const { return coeff[i]; }
    ...
    template<typename T>
    CPPGL_HD Eigen::Quaternion<T> operator*(
    ...
    public:
        CPPGL_HD DualQuaternionBase()
                : q0(T(1), T(0), T(0), T(0)),
                  qe(T(0), T(0), T(0), T(0)) {

        };

        CPPGL_HD DualQuaternionBase(T q0x, T q0y, T q0z, T q0w, T qex,
                                    T qey, T qez, T qew)
                : q0(q0w, q0x, q0y, q0z),
                  qe(qew, qex, qey, qez) {
        ...
        CPPGL_HD DualQuaternionBase(const Eigen::Quaternion<T> &q0,
        ...
        CPPGL_HD DualQuaternionBase(const Eigen::Vector<T, 4> &q0,
                                    const Eigen::Vector<T, 3> &t)
                : q0(q0) {
        ...
        CPPGL_HD DualQuaternionBase(const Eigen::Vector<T, 3> &point)
                : q0(T(1), T(0), T(0), T(0)),
                  qe(T(0), point.x(), point.y(), point.z()) {

        };

        CPPGL_HD DualQuaternionBase(const Eigen::Matrix<T, 4, 4> &Rt) {
        ...
        CPPGL_HD DualQuaternionBase(Eigen::Quaternion<T> &q0,
                                    Eigen::Quaternion<T> &qe)
                : q0(q0), qe(qe) {};

        static CPPGL_HD inline DualQuaternionBase<T> Zero() {
            return DualQuaternionBase<T>(T(0), T(0), T(0), T(0), T(0), T(0), T(0),
                                         T(0));
        }

        static CPPGL_HD inline DualQuaternionBase<T> Identity() {
            return DualQuaternionBase<T>(T(0), T(0), T(0), T(1), T(0), T(0), T(0),
                                         T(0));
        }

        CPPGL_HD Eigen::Quaternion<T> real() const { return q0; }

        CPPGL_HD Eigen::Quaternion<T> dual() const { return qe; }

        CPPGL_HD void norm(T &n1, T &n2) const {
            n1 = q0.norm();
        ...
        CPPGL_HD DualQuaternionBase<T> normalized() const {
        ...
        CPPGL_HD DualQuaternionBase<T> inverse() const {
        ...
        CPPGL_HD DualQuaternionBase<T> quaternion_conjugate() const {
            DualQuaternionBase<T> result;
        ...
        CPPGL_HD DualQuaternionBase<T> conjugate() const {
            DualQuaternionBase<T> result;
            result.q0 = q0.conjugate();
        ...
        template<typename S>
        CPPGL_HD DualQuaternionBase<S> cast() const {
            DualQuaternionBase<S> result(q0.template cast<S>(), qe.template cast<S>());
            return result;
        }

        CPPGL_HD bool isNormalized() const {
            T n1, n2;
            norm(n1, n2);
        ...
        CPPGL_HD friend Eigen::Vector<T, 3> operator*(
        ...
        CPPGL_HD Eigen::Matrix<T, 4, 4> matrix() const {
        ...
        CPPGL_HD Eigen::Vector<T, 7> toQuatTrans() const {
...
// instantiation
    template
    class _API cppgl::DualQuaternionBase<float>;

    CPPGL_HOST DualQuaternion DQB(const std::vector<float> &weights,
                                  const std::vector<cppgl::DualQuaternion> &quats);

    template<typename T>
    CPPGL_HOST void DQB(const T *weights, const T (*quats)[8], const int n,
                        T *output) {
        for (int i = 0; i < n; i++) {
...
// replace lie_jacobi by zero Matrix
    CPPGL_HOST Eigen::Matrix<float, 8, 6> DQBJacobian(
    ...
    CPPGL_DEVICE Eigen::Matrix<float, 8, 6> DQBJacobian(
            const float *weights, const cppgl::DualQuaternion *quats,
            const unsigned int size, const unsigned int derivative_axis,
            const Eigen::Matrix<float, 8, 6> &lie_jacobi =
            Eigen::Matrix<float, 8, 6>::Zero(8, 6));

    CPPGL_HOST mat4 DQB(const std::vector<float> &weights,
                        const std::vector<mat4> &transforms);

    CPPGL_DEVICE mat4 DQB(const float *weights, const mat4 *transforms,
                          const size_t size);

    CPPGL_DEVICE cppgl::DualQuaternion DQB(const float *weights,
                                           const cppgl::DualQuaternion *transforms,
                                           const size_t size);
