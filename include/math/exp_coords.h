...
// representation for rotations
    ...
    public:
        inline CPPGL_HD AxisAngleBase() : w_(T(0), T(0), T(0)) {};

        inline CPPGL_HD AxisAngleBase(const Eigen::Vector<T, 3> &w) : w_(w) {};

        inline CPPGL_HD AxisAngleBase(const Eigen::Map<Eigen::Vector<T, 3>> &w)
                : w_(w) {};

        inline CPPGL_HD AxisAngleBase(const Eigen::Matrix<T, 4, 4> &mat)
                : w_(T(0), T(0), T(0)) {
            ...
            *this = AxisAngleBase<T>(quat);
            ...
            // #if HAS_CERES_MODULE
            //     T theta = ceres::acos((mat.trace() - T(1.f)) / T(2.f));
            // #else
            ...
            //       unsigned int nonzero_entries = abs(cppgl::sign(w_[0])) +
            //                                      abs(cppgl::sign(w_[1])) +
            //                                      abs(cppgl::sign(w_[2]));
            ...
            //           if (w_[0] == T(0)) {
            //             w_[1] *= cppgl::sign(mat(1, 2));
            //           } else if (w_[1] == T(0)) {
            //             w_[0] *= cppgl::sign(mat(0, 2));
            //           } else if (w_[2] == T(0)) {
            //             w_[1] *= cppgl::sign(mat(0, 1));
            //           }
            //         } else if (nonzero_entries == 3) {
            //           if (w_[0] > T(0)) {
            //             w_[1] *= cppgl::sign(mat(0, 1) / w_[0]);
            //             w_[2] *= cppgl::sign(mat(1, 2) * (w_[0] / mat(0, 1)));
            //             if (mat(0, 2) > T(0)) {
            //               w_[0] *= cppgl::sign(w_[2]);
            //             } else {
            //               w_[0] *= -cppgl::sign(w_[2]);
            //             }
            //           } else if (w_[1] > T(0)) {
            //             w_[0] *= cppgl::sign(mat(0, 1) / w_[1]);
            //             w_[2] *= cppgl::sign(mat(0, 2) * (w_[1] / mat(0, 1)));
            //             if (mat(1, 2) > T(0)) {
            //               w_[1] *= cppgl::sign(w_[2]);
            //             } else {
            //               w_[1] *= -cppgl::sign(w_[2]);
            //             }
            //           } else if (w_[2] > T(0)) {
            //             w_[0] *= cppgl::sign(mat(0, 2) / w_[2]);
            //             w_[1] *= cppgl::sign(mat(0, 1) * (w_[2] / mat(0, 2)));
            //             if (mat(1, 2) > T(0)) {
            //               w_[2] *= cppgl::sign(w_[0]);
            //             } else {
            //               w_[2] *= -cppgl::sign(w_[0]);
            //             }
            //           }
...
#if HAS_CERES_MODULE
        inline CPPGL_HD AxisAngleBase(const Eigen::Quaternion<T> &q)
            : w_(T(0), T(0), T(0))
        {
          T theta = ceres::acos(q.coeffs().w()) * T(2);

          T sin_theta = ceres::sin(theta / T(2.f));
          int is_zero = (ceres::abs(sin_theta) < T(1e-8f));
          w_[0] = T(1 - is_zero) * (q.x() / (T(cppgl::sign_nonzero(sin_theta)) *
                                             MAX(abs(sin_theta), T(1e-8f))));
          w_[1] = T(1 - is_zero) * (q.y() / (T(cppgl::sign_nonzero(sin_theta)) *
                                             MAX(abs(sin_theta), T(1e-8f))));
          w_[2] = T(1 - is_zero) * (q.z() / (T(cppgl::sign_nonzero(sin_theta)) *
                                             MAX(abs(sin_theta), T(1e-8f))));
          w_ *= theta;
        }
#endif

        inline CPPGL_HD T Angle() const {
        ...
        static inline CPPGL_HOST T Angle(T const *data) {
        ...
        inline CPPGL_HD Eigen::Matrix<T, 3, 3> ToMatrix() const {
        ...
        static inline CPPGL_HOST Eigen::Matrix<T, 3, 3> ToMatrix(T const *data) {
            Eigen::Map<const Eigen::Vector<T, 3>> w(data);
            Eigen::Matrix<T, 3, 3> CpuMat = Eigen::Matrix<T, 3, 3>::Identity(3, 3);
            T theta = Angle(data);
        ...
        inline CPPGL_HD Eigen::MatrixXf ToMatrixJacobi() const {
        ...
        inline CPPGL_HD Eigen::Quaternion<T> ToQuaternion() const {
            T theta = Angle();
        ...
        static inline void CPPGL_HOST ToQuaternion(T const *data, T *output) {
            T theta = Angle(data);
        ...
        inline CPPGL_HD Eigen::Matrix<float, 4, 3> ToQuaternionJacobi() const {
        ...
        inline CPPGL_HD Eigen::Vector<T, 3> Axis() const {
            T theta = T(0);
            if (w_.cwiseAbs().sum() > T(0)) {
                theta = w_.norm();
            }
        ...
        static inline CPPGL_HOST Eigen::Vector<T, 3> Axis(T const *data) {
            Eigen::Map<const Eigen::Vector<T, 3>> w(data);
        ...
        inline CPPGL_HD Eigen::Vector<T, 3> Data() const { return w_; }

        inline CPPGL_HD Eigen::Matrix<T, 3, 3> SkewMatrix() const {
        ...
        static inline CPPGL_HOST Eigen::Matrix<T, 3, 3> SkewMatrix(T const *data) {
            Eigen::Map<const Eigen::Vector<T, 3>> w(data);
        ...
        CPPGL_HD T operator[](const int i) const { return w_[i]; }

        CPPGL_HD friend AxisAngleBase operator+(const AxisAngleBase &q1,
                                                const AxisAngleBase &q2) {
            Eigen::Vector<T, 3> w = q1.w_ + q2.w_;
            return AxisAngleBase(w);
        }

        CPPGL_HD friend AxisAngleBase &operator+=(AxisAngleBase &q1,
                                                  const AxisAngleBase &q2) {
            q1.w_ += q2.w_;
            return q1;
        }
        ...
        // w itself
        inline CPPGL_HD Eigen::Matrix<T, 3, 3> PartialDerivSkewW(
                unsigned int derivative_axis) const {
        ...
        // have a cppgl::sign twist)
        inline CPPGL_HD Eigen::Matrix<T, 3, 3> PartialDerivSkew2W(
                unsigned int derivative_axis) const {
        ...
        inline CPPGL_HD Eigen::Matrix<T, 3, 3> PartialDerivWwt(
                unsigned int derivative_axis) const {
        ...
        inline CPPGL_HD T PartialDerivAngle(unsigned int derivative_axis) const {
            T norm = Angle();
            int is_zero = norm < 1e-8f;
        ...
        inline CPPGL_HD Eigen::Matrix<T, 3, 3> PartialDerivRot(
                unsigned int derivative_axis) const {
            T theta = Angle();
...
// representation for affine transformations
    ...
    public:
        inline CPPGL_HD TwistBase(bool pseudo = false) : v_(0, 0, 0) {};

        inline CPPGL_HD TwistBase(const Eigen::Vector<T, 3> w,
                                  const Eigen::Vector<T, 3> v, bool pseudo = false)
                : w_(w), v_(v), pseudo_(pseudo) {};

        inline CPPGL_HD TwistBase(const AxisAngleBase<T> w,
                                  const Eigen::Vector<T, 3> v, bool pseudo = false)
                : w_(w), v_(v), pseudo_(pseudo) {};

        // TODO: Matrix inverse is no CPPGL_DEVICE function?
        CPPGL_HOST TwistBase(const Eigen::Matrix<T, 4, 4> &CpuMat,
                             bool pseudo = false)
        ...
        CPPGL_HD TwistBase(const DualQuaternion &q, bool pseudo = false)
        ...
        inline CPPGL_HD Eigen::Matrix<T, 4, 4> ToMatrix() const {
        ...
        static inline CPPGL_HOST Eigen::Matrix<T, 4, 4> ToMatrix(
        ...
        inline CPPGL_HD Eigen::Vector<T, 3> TranslationVector() const {
            T theta = w_.Angle();
        ...
        static inline CPPGL_HOST Eigen::Vector<T, 3> TranslationVector(
                T const *data) {
            Eigen::Map<const Eigen::Vector<T, 3>> v(data + 3);
            T theta = AxisAngleBase<T>::Angle(data);
        ...
        inline CPPGL_HD DualQuaternionBase<T> ToDualQuaternion() const {
            T theta = w_.Angle();
        ...
        static inline CPPGL_HOST void ToDualQuaternion(T const *data, T *output,
                                                       bool pseudo = false) {
        ...
        inline CPPGL_HD Eigen::Matrix<float, 8, 6> ToDualQuaternionJacobi() const {
        ...
        inline CPPGL_HD Eigen::Vector<float, 6> Data() const {
        ...
        inline CPPGL_HD AxisAngleBase<T> Rot() const { return w_; }

        inline CPPGL_HD Eigen::Vector<T, 3> Trans() const { return v_; }
        ...
        CPPGL_HD T operator[](const int i) const {
            if (i <= 2) {
                return w_[i];
            } else {
                return v_[i - 3];
        ...
        CPPGL_HD friend TwistBase<T> operator+(const TwistBase<T> &q1,
        ...
        CPPGL_HD friend TwistBase<T> &operator+=(TwistBase<T> &q1,
                                                 const TwistBase<T> &q2) {
            q1.w_ += q2.w_;
            q1.v_ += q2.v_;
            return q1;
    ...
    private:
        ...
        CPPGL_HD bool IsPureTrans() const {
        ...
        inline CPPGL_HD Eigen::Vector<T, 3> PartialDerivT(
                unsigned int derivative_axis) const {
            T norm = w_.Angle();
            int pure_trans = IsPureTrans();
