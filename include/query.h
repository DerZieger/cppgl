...
// Query interface (with ring buffer)
    ...
    class Query {
    public:
        Query(const std::string &name, size_t N = 256) : name(name), N(N), curr(0), data(N, 0.f), exp_avg(0.f),
                                                         last_val(0.f) {}
...
// (CPU) TimerQuery (in ms)
    ...
    template
    class _API NamedHandle<TimerQueryImpl>; // needed for Windows DLL export
...
// (GPU) TimerQueryGL (in ms)
    ...
    template
    class _API NamedHandle<TimerQueryGLImpl>; // needed for Windows DLL export
...
// (GPU) PrimitiveQueryGL
    ...
    template
    class _API NamedHandle<PrimitiveQueryGLImpl>; // needed for Windows DLL export
...
// (GPU) FragmentQueryGL
    ...
    template
    class _API NamedHandle<FragmentQueryGLImpl>; // needed for Windows DLL export
