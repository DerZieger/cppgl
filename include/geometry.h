...
// Geometry
    ...
    public:
        ...
        virtual void set(const std::vector<float> &positions, const std::vector<uint32_t> &indices, const std::vector<float> &normals = std::vector<float>(),  const std::vector<float> &texcoords = std::vector<float>());
    ...
    public:
        ...
        void set(const std::vector<float> &positions,
                 const std::vector<uint32_t> &indices,
                 const std::vector<float> &normals = std::vector<float>(),
                 const std::vector<float> &texcoords = std::vector<float>());
    ...
    public:
        ...
        vec3 get_position(unsigned int index) const override {
            _CPPGL_ASSERT_LT(index, positions_size_);
            return make_vec3(&positions_ptr_[index * 3]);
        }

        vec3 get_normal(unsigned int index) const override {
            _CPPGL_ASSERT_LT(index, normals_size_);
            return make_vec3(&normals_ptr_[index * 3]);
        }

        vec2 get_texcoord(unsigned int index) const override {
            _CPPGL_ASSERT_LT(index, texcoords_size_);
            return make_vec2(&texcoords_ptr_[index * 2]);
        }

        uint32_t get_index(unsigned int index) const override {
            _CPPGL_ASSERT_LT(index, indices_size_);
    ...
    template
    class _API NamedHandle<GeometryBaseImpl>;  // needed for Windows DLL export
    template
    class _API NamedHandle<GeometryImpl>;  // needed for Windows DLL export
    template
    class _API NamedHandle<GeometryWrapperImpl>;  // needed for Windows DLL export
